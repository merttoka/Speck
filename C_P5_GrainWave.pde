  ...
  public void draw(PGraphics pg) {
    ...
    if(selection[0] >= 0 && selection[1] >= 0 && isDrawGrainWave){
      ...
      for( int i = 0; i < grain_samples.length - 1; i++ )
      {
        ...
        float e1 = envelope_values[(int)map(i, 0, grain_samples.length, 0, envelope_values.length)];
        float e2 = envelope_values[(int)map(i+1, 0, grain_samples.length, 0, envelope_values.length)];
